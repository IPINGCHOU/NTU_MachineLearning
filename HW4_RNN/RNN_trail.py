...
#%%
# get some infos for train x and y
toarray_y = np.array(y).astype(int)
print(np.sum(toarray_y)) # 100045
all_train_x_len=[]
for i in train_x:
    all_train_x_len.append(len(i))

import matplotlib.pyplot as plt
fig, ax = plt.subplots(figsize=(25, 25))
plt.hist(np.array(all_train_x_len))
plt.xticks(size = 40)
plt.yticks(size = 40)
plt.show()
# %%
...
epoch = 20
lr = 0.001
MODEL_NAME = 'LSTM6_hidden500_drop0.1_lr0.001'
...
# %%
# check the confusion matrix on valid data set
THRESHOLD = 0.5
batch_size=512
MODEL_NAME = 'COMBINED_LSTM6_hidden500_drop0.5_LSTMdrop0.1_lr0.001_nolabel0.2torr'
model = torch.load(os.path.join(model_dir, MODEL_NAME+'.model'))
outputs, gt = testing(batch_size, val_loader, model, device, THRESHOLD, flag='valid')

# %%
import matplotlib.pyplot as plt
from sklearn.metrics import confusion_matrix
confmat = confusion_matrix(y_true=gt, y_pred=outputs, normalize='true')
...
# %%
# get outputs between 1 and 0 THRESHOLDs
# no_label_outputs = outputs
# hide above 
THRESHOLD_one = 0.8
THRESHOLD_zero = 0.2
outputs = np.array(no_label_outputs)
...
#%%
# combine the labeld x and semi labeled x
train_x, y = load_training_data(train_with_label)
...
epoch = 50
lr = 0.001
MODEL_NAME = 'COMBINED_LSTM4_hidden512_drop0.2_LSTMdrop0.1_lr0.001_nolabel0.2torr'
model = LSTM_Net(embedding, embedding_dim=300,
                            hidden_dim=512,
                            num_layers=4,
                            dropout=0.2,
...
# tmp.to_csv(os.path.join(path_prefix, 'predict_'+MODEL_NAME+'.csv'), index=False)
# print("Finish Predicting")

tmp.to_csv(os.path.join(path_prefix, 'predict_check2.csv'), index=False)
print("Finish Predicting")

# %%
# for report q2

def testing_q2(sentence):
    model.eval()
...
print(testing_q2([sent1]))
print(testing_q2([sent2]))
